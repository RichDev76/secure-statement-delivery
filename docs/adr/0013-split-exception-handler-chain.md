...
`GlobalExceptionHandler` was one `@RestControllerAdvice` that imported exception types from every
feature — exactly the cross-feature coupling
[0011](0011-adopt-feature-first-hexagonal-packaging.md) avoids everywhere else. Its catch-all
mapped any unmapped `RuntimeException`/`Exception` to a 400/500 with `ex.getMessage()` attached
verbatim. That leaked a server filesystem path in one case and PII (account number, IP,
user-agent) in another, and no test proved any exception mapped to the right, safe response.
...
The catch-all is now safe and `ErrorResponse`-aware. A framework-classified 4xx still exposes its
message, but every other case — including a misclassified 5xx — logs at ERROR server-side and
returns a fixed generic detail instead. The old blanket "unmapped → 400 with raw message" handler
is retired.
