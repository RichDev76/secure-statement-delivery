...
public class S3StatementFileStore implements StatementFileStore {
    ...
    @Override
    public String store(
            UUID id, String accountNumber, LocalDate statementDate, long contentLength, StreamSupplier content)
            throws IOException {
        ...
        // The SDK only closes the last stream a provider hands out, so this closes its own on retry.
        ...
        try {
            s3Client.putObject(
                    PutObjectRequest.builder()
                            ...
                            .key(key)
                            .build(),
                    RequestBody.fromContentProvider(provider, contentLength, "application/octet-stream"));
    ...
    private static void closeQuietly(InputStream stream) {
        ...
        try {
            ...
            log.debug("Failed to close a superseded retry attempt's source stream", e);
        }
    }
